...
Raon is a 2-stage streaming pipeline:
  ...
  Stage 1: Mimi codec-to-waveform decode
"""
...
# Model identifier – placeholder path following the local-checkpoint convention
# used by Raon (the model_type is "raon", not a HF-hub slug).
MODEL = "/path/to/Raon"
            ...
            "stage_args": {
                0: {
                    "engine_args.custom_process_next_stage_input_func": (
                        ...
                        ".raon.stage0_to_stage1_async_chunk"
                    )
                },
            },
        },
...
def test_text_to_audio_stream(omni_server, openai_client) -> None:
    """
    Test text input processing and audio output generation via OpenAI API (streaming).
...
class TestRaonSpeechEndpoint:
    ...
    def test_speech_basic(self, omni_server) -> None:
        ...
        assert len(response.content) > MIN_AUDIO_BYTES, (
            f"Audio content too small ({len(response.content)} bytes), "
            f"expected at least {MIN_AUDIO_BYTES} bytes"
        )
