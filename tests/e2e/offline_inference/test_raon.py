...
def test_text_to_text(omni_runner, omni_runner_handler) -> None:
    """
    Test text input processing and text output generation via offline inference.
    Deploy Setting: CI yaml (single GPU, async_chunk disabled)
    Input Modal: text
    Output Modal: text
    Input Setting: stream=False
...
@hardware_test(res={"cuda": "L4"}, num_cards=1)
@pytest.mark.parametrize("omni_runner", test_params, indirect=True)
def test_text_to_text_and_audio(omni_runner, omni_runner_handler) -> None:
    """
    Test text input processing and combined text + audio output generation.
    Deploy Setting: CI yaml (single GPU, async_chunk disabled)
    Input Modal: text
