...
class StreamingMimiModel(MimiModel):
    ...
    def _init_conv1d_padding_cache(self) -> MimiConv1dPaddingCache:
        ...
        for layer_name in self.decoder._mimiconv1d_layer_names:
            ...
            per_layer_padding.append(layer.padding_total)  # type: ignore[attr-defined]
        ...
        return MimiConv1dPaddingCache(
            ...
            per_layer_padding=per_layer_padding,  # type: ignore[arg-type]
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        """Load both parameters and persistent buffers.

        vLLM's `AutoWeightsLoader` only loads parameters (and a small set of
        special-case buffers like BatchNorm stats). Mimi codebooks store their
        embedding table in persistent buffers (`embed_sum`, `cluster_usage`,
        `initialized`), so skipping buffers yields incorrect quantizer decode
        and severely degraded / noisy audio.
        """
    ...
    def decode(  # type: ignore
        ...
        if use_streaming and conv1d_padding_cache is None:
            conv1d_padding_cache = self._init_conv1d_padding_cache()

        if use_streaming and convtranspose1d_padding_cache is None:
            convtranspose1d_padding_cache = self._init_convtranspose_padding_cache()
