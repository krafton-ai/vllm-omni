...
class RaonCodePredictor(nn.Module):
    ...
    def predict_codes(
        ...
        return self.forward(
            layer0_code, layer0_embed, last_hidden,
...
class RepetitionAwareSampler:
    """Per-model RAS state manager."""

    def __init__(self) -> None:
        self._code_history: dict[str, list[int]] = defaultdict(list)
    ...
    def maybe_resample(
        ...
        if not ENV.ras_enabled:
            return first_code
        if req_id is None or first_code >= codebook_size:
            return first_code

        hist = self._code_history.get(req_id)
        if not hist:
            return first_code

        window = hist[-ENV.ras_window_size :]
        if not window:
            return first_code
        ...
        logger.debug(
            "[RAS] req=%s resampled code %d -> %d (ratio=%.3f window=%d)",
            req_id,
            old_code,
            first_code,
            ratio,
    ...
    def record(self, req_id: str | None, first_code: int) -> None:
        if ENV.ras_enabled and req_id is not None:
            self._code_history[req_id].append(first_code)

    def cleanup(self, req_id: str) -> None:
        self._code_history.pop(req_id, None)
